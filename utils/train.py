...
def train_rnn_model_with_parameters_regularzition(
    embedding_matrix: np.ndarray,
    train_dataset: Dataset,
    val_dataset: Dataset,
    batch_size: int,
    ...
    show_progress: bool = True,
    seed: int = 42,
    log_dir: str = "rnn/regularization",
    early_stopping_patience: int = 3,
    freeze_embedding: bool = True,
    rnn_type: str = "RNN",
    bidirectional: bool = False,
    ...
    embedding_dropout: float = 0.0,
    weight_decay: float = 0.0,
    regularization_type: str = "unknown",
):
    ...
    # Train model.
    log_file_name = f"{log_dir}/batch_size_{batch_size}-lr_{learning_rate}-optimizer_{optimizer_name}-hidden_dim_{hidden_dim}-num_layers_{num_layers}-sr_type_{sentence_representation_type}-freeze_{freeze_embedding}-rnn_type_{rnn_type}-bidirectional_{bidirectional}-regularization_{regularization_type}"
    ...
    trainer = L.Trainer(
        default_root_dir="models/",
        callbacks= callbacks if regularization_type != "unknown" else callbacks,
        min_epochs=min_epochs,
        max_epochs=max_epochs,
        logger=logger,
        accelerator="auto",
